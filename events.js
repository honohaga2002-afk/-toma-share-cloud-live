...
const EVENTS=[
  ...
  {date:'9/12(土)〜9/13(日)',time:'9:30〜20:00（予定）',title:'とまこまいミライフェスト2026',place:'キラキラ公園',tag:'大型イベント',url:'https://www.city.tomakomai.hokkaido.jp/kouhou/kouhou_yomiage.php?id=288&yid='},
  ...
  {date:'9/26(土)',time:'10:00〜12:30',title:'公園の「なぜ？」を見つけ隊！',place:'出光カルチャーパーク・中央図書館',tag:'自然・子ども',url:'https://www.city.tomakomai.hokkaido.jp/shizen/shizenhogo/seibutsu/mituketai.html'},
  ...
  {date:'10/18(土)〜10/19(日)',time:'18日13:00〜16:00／19日9:00〜12:00',title:'樽前地区文化祭',place:'樽前小学校',tag:'地域・小規模',url:'https://www.city.tomakomai.hokkaido.jp/kyoiku/shogaigakushu/bunka/eventannai/siminbunkasai.html'},
...
function renderEvents(){
  ...
  if(cal.querySelector('#tomakomaiEventList')) return;

  const box=document.createElement('div');
  box.id='tomakomaiEventList';
  box.className='card';
  box.style.marginTop='14px';
  box.innerHTML=`
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:10px">
      <div><div class="title" style="font-size:19px">🎪 苫小牧イベント情報</div><div class="meta">小規模な地域行事・教室・展示も掲載／開催日時つき</div></div>
      <span class="pill">${EVENTS.length}件</span>
    </div>
    <div style="display:grid;gap:9px">
      ${EVENTS.map(e=>`<a href="${esc(e.url)}" target="_blank" rel="noopener noreferrer" style="display:block;text-decoration:none;color:inherit;border:1px solid #dbe8ef;border-radius:12px;padding:12px;background:#fff">
        <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start"><strong style="font-size:15px;line-height:1.4">${esc(e.title)}</strong><span class="pill" style="white-space:nowrap">${esc(e.tag)}</span></div>
        <div style="margin-top:7px;font-weight:800;color:#0b76a8">📅 ${esc(e.date)}　⏰ ${esc(e.time)}</div>
        <div class="meta" style="margin-top:5px">📍 ${esc(e.place)}　› 公式情報</div>
      </a>`).join('')}
    </div>
    <a class="btn" href="https://www.city.tomakomai.hokkaido.jp/calendar/" target="_blank" rel="noopener noreferrer" style="display:block;text-align:center;margin-top:12px;text-decoration:none">苫小牧市 行事カレンダーを確認</a>`;
  cal.appendChild(box);
}

const observer=new MutationObserver(()=>setTimeout(renderEvents,0));
observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});
document.addEventListener('click',()=>setTimeout(renderEvents,0));
window.addEventListener('load',()=>setTimeout(renderEvents,300));
})();
