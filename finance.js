...
let tab='dashboard';
let bulkRows=Array.from({length:8},()=>({date:'',type:'expense',accountId:'venue',amount:'',description:'',payment:'現金'}));
...
function openBudget(){
  ...
  document.querySelectorAll('.nav').forEach(n=>n.classList.toggle('on',n.hasAttribute('data-budget-nav')));render();window.scrollTo({top:0,behavior:'smooth'});
}
...
const tabs=[['dashboard','概要'],['quick','かんたん入力'],['bulk','連続入力'],['transactions','取引一覧'],['budget','予算書'],['accounts','勘定科目'],['carry','年度繰越']];
function render(){
...
function renderTab(d){const b=$('tbBody');if(tab==='dashboard')dashboard(b,d);else if(tab==='quick')quick(b,d);else if(tab==='bulk')bulk(b,d);else if(tab==='transactions')transactions(b,d);else if(tab==='budget')budget(b,d);else if(tab==='accounts')accounts(b,d);else carry(b,d);}
...
function bulk(b,d){
  const row=(r,i)=>`<tr><td><input type="date" data-bi="${i}" data-bf="date" value="${esc(r.date)}"></td><td><select data-bi="${i}" data-bf="type"><option value="expense" ${r.type==='expense'?'selected':''}>支出</option><option value="income" ${r.type==='income'?'selected':''}>収入</option></select></td><td><select data-bi="${i}" data-bf="accountId">${opts(d,r.type,r.accountId)}</select></td><td><input type="number" inputmode="numeric" data-bi="${i}" data-bf="amount" value="${esc(r.amount)}"></td><td><input data-bi="${i}" data-bf="description" value="${esc(r.description)}"></td><td><select data-bi="${i}" data-bf="payment">${['現金','振込','カード','電子決済','その他'].map(x=>`<option ${x===r.payment?'selected':''}>${x}</option>`).join('')}</select></td><td><button class="tbBtn danger" data-bdel="${i}">削除</button></td></tr>`;
  b.innerHTML=`<div class="tbPanel"><h3>連続入力</h3><div class="tbNote">複数件を続けて入力し、最後にまとめて反映します。保存後は取引一覧へ日付順に並びます。</div><div class="tbTableWrap"><table class="tbTable tbBulkTable"><tr><th>日付</th><th>区分</th><th>勘定科目</th><th>金額</th><th>内容</th><th>方法</th><th></th></tr>${bulkRows.map(row).join('')}</table></div><div class="tbBulkActions"><button id="tbBulkAdd" class="tbBtn light">入力行を追加</button><button id="tbBulkSave" class="tbBtn">まとめて反映</button></div></div>`;
  b.querySelectorAll('[data-bi]').forEach(x=>x.onchange=()=>{const i=Number(x.dataset.bi),f=x.dataset.bf;bulkRows[i][f]=x.value;if(f==='type'){bulkRows[i].accountId=d.accounts.find(a=>a.type===x.value)?.id||'';bulk(b,d);}});
  b.querySelectorAll('[data-bdel]').forEach(x=>x.onclick=()=>{bulkRows.splice(Number(x.dataset.bdel),1);if(!bulkRows.length)bulkRows.push({date:'',type:'expense',accountId:d.accounts.find(a=>a.type==='expense')?.id||'',amount:'',description:'',payment:'現金'});bulk(b,d);});
  $('tbBulkAdd').onclick=()=>{bulkRows.push({date:'',type:'expense',accountId:d.accounts.find(a=>a.type==='expense')?.id||'',amount:'',description:'',payment:'現金'});bulk(b,d);};
  $('tbBulkSave').onclick=()=>{const filled=bulkRows.filter(r=>r.date||r.amount||r.description);if(!filled.length){alert('入力された取引がありません');return;}if(filled.some(r=>!r.date||!Number(r.amount))){alert('入力した行の日付と金額を確認してください');return;}const now=Date.now();filled.forEach((r,i)=>d.transactions.push({id:(now+i).toString(36)+Math.random().toString(36).slice(2),date:r.date,type:r.type,accountId:r.accountId,amount:Number(r.amount),description:r.description.trim(),payment:r.payment,memo:''}));d.transactions.sort((a,b)=>(a.date||'').localeCompare(b.date||''));save(d);bulkRows=Array.from({length:8},()=>({date:'',type:'expense',accountId:d.accounts.find(a=>a.type==='expense')?.id||'',amount:'',description:'',payment:'現金'}));tab='transactions';render();};
}
...
document.querySelector('[data-budget-nav]')?.addEventListener('click',openBudget);
