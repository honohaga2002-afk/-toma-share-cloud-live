...
const loginEl=$('login'),
      codeEl=$('code'),
      memberNameEl=$('memberName'),
      enterEl=$('enter'),
      statusEl=$('loginStatus'),
      navEl=$('nav'),
      toastEl=$('toast');
...
async function doLogin(){
  ...
  try{
    ...
    statusEl.className='err';
    statusEl.textContent='ログインできません：'+e.message;
  }finally{
...
codeEl.value=
  localStorage.getItem('tomaCode')||'TOMA-2026';
...
function dataUrlToBlob(dataUrl){
  ...
  if(!m)
    throw new Error('ファイル形式を読み取れません');
...
function driveR(){
  ...
  `<div class="panel">
    ...
    <div class="meta">
      ExcelはGoogleスプレッドシート、
      WordはGoogleドキュメント、
      PowerPointはGoogleスライドへ
      自動変換してGoogle Driveに保存します。
      PDF・画像にも対応しています。
    </div>
...
function calR(){
  ...
  $('addS').onclick=async()=>{
    ...
    if(!date)
      return alert('日付を選んでください');

    if(!title)
      return alert('予定名を入力してください');
